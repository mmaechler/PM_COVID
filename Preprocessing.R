...
covid_us_hist <- subset(covid_hist, Country_Region == "US" & is.na(FIPS) == FALSE)
...
covid_us <- subset(covid,Country_Region == "US" & is.na(FIPS) != TRUE)
covid_us <- rbind(covid_us,subset(covid_us_hist, (!(FIPS %in% covid_us$FIPS))  & Confirmed == 0 & Deaths == 0 & is.na(FIPS) == FALSE))
...
county_base_mortality <- merge(county_base_mortality,county_old_mortality[,c(2,4)],
                               by = "County.Code")
county_base_mortality$older_pecent <-
    county_base_mortality$older_Population / county_base_mortality$Population
...
county_pm_aggregated <- merge(county_pm_aggregated,
                              county_temp_aggregated, by = "fips", all.x = TRUE)
...
aggregate_pm_census = merge(aggregate_pm,county_census_aggregated2,
                            by.x = "fips", by.y = "fips")

aggregate_pm_census_cdc = merge(aggregate_pm_census,county_base_mortality[,c(1,4:5,8:9)],
                                by.x = "fips",by.y = "County.Code", all.x = TRUE)

aggregate_pm_census_cdc <-
    aggregate_pm_census_cdc[is.na(aggregate_pm_census_cdc$fips) == FALSE,]

aggregate_pm_census_cdc_test = merge(aggregate_pm_census_cdc, state_test,
...
## This is the final version of the data used later
aggregate_pm_census_cdc_test_beds <-
    merge(aggregate_pm_census_cdc_test,
          county_hospitals_aggregated,
          by.x = "fips",by.y = "COUNTYFIPS", all.x = TRUE)

## Save this data, for easy access:
saveRDS(aggregate_pm_census_cdc_test_beds,
        file = "Preprocessed_aggreg_pm_cdc.rds")
